...
from app.schemas.chat import (ChatResponse, ChatResponseWithMessages,
                              MessageResponse)
...
def get_chat(chat_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if user.role == "Farmer":
        ...
        if other_user.role != "Buyer":
            raise HTTPException(
                status_code=403, detail="Farmers cannot communicate with other farmers")
        ...
        if other_user.role != "Farmer":
            raise HTTPException(
                status_code=403, detail="Buyers cannot communicate with other buyers")
    else:
...
async def websocket_chat(websocket: WebSocket, chat_id: int, db: Session = Depends(get_db)):
    ...
    # logger.info(
    #     f"Authenticated WebSocket user: user_id={user.id}, chat_id={chat_id}")
    ...
    logger.info(
        f"WebSocket connection established: user_id={user.id}, chat_id={chat_id}")
...
def create_chat_with_farmer(farmer_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    logger.info(
        f"Starting to create chat with farmer_id={farmer_id} for user_id={user.id}, role={user.role}")

    if user.role != "Buyer":
        logger.error(f"Permission denied: user_id={user.id} is not a Buyer")
        raise HTTPException(
            status_code=403, detail="Only buyers can initiate chats")
    ...
    existing_chat = chat_repository.get_chat_between_users(
        db=db, buyer_id=user.id, farmer_id=farmer_id)
    if existing_chat:
        logger.info(
            f"Existing chat found: chat_id={existing_chat.id} for user_id={user.id}")
        return ChatResponse.from_orm(existing_chat)

    new_chat = chat_repository.create_chat(
        db=db, buyer_id=user.id, farmer_id=farmer_id)
