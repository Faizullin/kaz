...
from app.schemas.password_reset import PasswordResetInitiate, PasswordResetConfirm
from app.schemas.users import UserCreate, UserLogin, UserUpdate, UserInfo
...
# ):
#     # Verify the code
#     is_valid = users_repository.verify_code(db, str(user_input.email), user_input.code, "registration")
#     if not is_valid:
...
@router.post("/auth/users", status_code=200)
...
@router.get("/auth/users/me", response_model=UserInfo, status_code=200)
...
@router.get("/auth/users/{user_id}", response_model=UserInfo, status_code=200)
...
@router.post("/auth/users/password-reset/initiate", status_code=200)
...
@router.post("/auth/users/password-reset/confirm", status_code=200)
